...
class TestManifest(unittest.TestCase):
    ...
    def test_load_manifest_has_all_tests(self):
        self.assertEqual(set(self.manifest.tests), set(range(3, 27)))

    def test_suite_membership(self):
        self.assertEqual(self.manifest.suites['fast']['tests'], [3, 4, 5, 7, 9, 12, 13, 14, 15])
        self.assertEqual(self.manifest.suites['minimal']['tests'],
                         [3, 4, 5, 6, 7, 8, 9, 10, 12, 13, 14, 15])
        self.assertEqual(self.manifest.suites['all']['tests'], list(range(3, 27)))

    def test_resolve_suite_and_alias(self):
        self.assertEqual(self.manifest.resolve_test_ids(['0']), list(range(3, 27)))
        self.assertEqual(self.manifest.resolve_test_ids(['101']), list(range(3, 27)))
        self.assertEqual(self.manifest.resolve_test_ids(['2']), [3, 4, 5, 7, 9, 12, 13, 14, 15])
    ...
    def test_build_help_text_contains_latest_test(self):
        self.assertIn('* 26', build_help_text())
