...
class Parameter(QObject):
    ...
    **Signals:**
    ...
    sigValueChanged = pyqtSignal(object, object)  # self, value   emitted when value is finished being edited
    sigValueChanging = pyqtSignal(object, object)  # self, value  emitted as value is being edited

    sigChildAdded = pyqtSignal(object, object, object)  # self, child, index
    sigChildRemoved = pyqtSignal(object, object)  # self, child
    sigRemoved = pyqtSignal(object)  # self
    sigParentChanged = pyqtSignal(object, object)  # self, parent
    sigLimitsChanged = pyqtSignal(object, object)  # self, limits
    sigDefaultChanged = pyqtSignal(object, object)  # self, default
    sigNameChanged = pyqtSignal(object, object)  # self, name
    sigOptionsChanged = pyqtSignal(object, object)  # self, {opt:val, ...}
    ...
    sigStateChanged = pyqtSignal(object, object, object)  # self, change, info

    # emitted when any child in the tree changes state
    # (but only if monitorChildren() is called)
    sigTreeStateChanged = pyqtSignal(object, object)  # self, changes
    # changes = [(param, change, info), ...]
    sigContextMenu = pyqtSignal(object, object)  # self, name
    ...
    def create(**opts):
        ...
        cls = Parameter if typ is None else PARAM_TYPES[opts['type']]
        return cls(**opts)
    ...
    def getValues(self):
        ...
        return {ch.name(): (ch.value(), ch.getValues()) for ch in self}
    ...
    def saveState(self, filter=None):
        ...
        ch = {ch.name(): ch.saveState(filter=filter) for ch in self}
        if ch:
            state['children'] = ch
        return state
    ...
    def emitStateChanged(self, changeDesc, data):
        # Emits stateChanged signal and requests emission of new treeStateChanged signal
    ...
    def insertChild(self, pos, child, autoIncrementName=None):
        ...
        if name in self.names and child is not self.names[name]:
            if autoIncrementName is not True and (
                    autoIncrementName is not None
                    or not child.opts.get('autoIncrementName', False)
            ):
